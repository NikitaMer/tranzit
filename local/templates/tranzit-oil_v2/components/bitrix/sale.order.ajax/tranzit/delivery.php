...
?>
    ...
    if(!empty($arResult["DELIVERY"]))
    {
        ...
        ?>
        <div class="field radio inline w480p">
            <label>Выберите способ доставки<?//=GetMessage("SOA_TEMPL_DELIVERY")?></label>
                ...
                foreach ($arResult["DELIVERY"] as $delivery_id => $arDelivery)
                {
                    ...
                    if ($delivery_id !== 0 && intval($delivery_id) <= 0)
                    {
                        foreach ($arDelivery["PROFILES"] as $profile_id => $arProfile)
                        {
                            ...
                            <label class="select" for="ID_DELIVERY_<?=$delivery_id?>_<?=$profile_id?>" onclick="BX('ID_DELIVERY_<?=$delivery_id?>_<?=$profile_id?>').checked=true;<?=$extraParams?>submitForm();">
                                    <div class="name"><?= htmlspecialcharsbx($arDelivery["TITLE"])?></div>
