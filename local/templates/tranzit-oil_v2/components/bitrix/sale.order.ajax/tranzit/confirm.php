...
if (!empty($arResult["ORDER"]))
{
	...
	#$rsPaySystem = PaySystemTable::getById($arResult["ORDER"]['PAY_SYSTEM_ID']);
	#$aPaySystem = $rsPaySystem->fetch();
...
<? if ($arResult["ORDER"]["PRICE"]>0 ) { ?>
	...
	<? if ($arOrder[SUM_PAID]>'0') { ?><li><span>Сумма к оплате c учетом<br> бонусов</span><b><?=$CSiteController->getHtmlFormatedPrice($arResult["ORDER"]["CURRENCY"], $price2);?></b></li><br> <? } ?>
				...
				<?/*
				<li><span>Бонус</span>потрачено 0 руб., будет зачисленно 45 руб.</li>
				<li><span>Итого</span><b><?=$CSiteController->getHtmlFormatedPrice($arResult["ORDER"]["CURRENCY"], $arResult["ORDER"]["PRICE"]);?></b></li>
				*/?>
