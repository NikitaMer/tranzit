...
?>
	...
	<form action="<?=POST_FORM_ACTION_URI?>" method="post" class="vote-form">
		...
		foreach ($arResult["QUESTIONS"] as $arQuestion)
		{
			$iCount++;
			?>
		<div class="question"><?=$arQuestion["QUESTION"]?><?/*if($arQuestion["REQUIRED"]=="Y"){echo "<span class='starrequired'>*</span>";}*/?></div>
		<div class="ansvers">
				...
				switch ($arAnswer["FIELD_TYPE"]):
					...
					case 0://radio
						...
						<div class="radio-block">
							<input type="radio" <?= $value ?> name="vote_radio_<?= $arAnswer["QUESTION_ID"] ?>" id="vote_radio_<?= $arAnswer["QUESTION_ID"] ?>_<?= $arAnswer["ID"] ?>" value="<?= $arAnswer["ID"] ?>" <?= $arAnswer["~FIELD_PARAM"] ?> />
							<span></span>
					...
					case 2://dropdown
						...
						<span class="vote-answer-item vote-answer-item-dropdown">
								<select
									name="vote_dropdown_<?= $arAnswer["QUESTION_ID"] ?>" <?= $arAnswer["~FIELD_PARAM"] ?>>
									<?foreach ($arAnswer["DROPDOWN"] as $arDropDown):?>
										<option
											value="<?= $arDropDown["ID"] ?>" <?= ($arDropDown["ID"] === $value) ? 'selected="selected"' : '' ?>><?= $arDropDown["MESSAGE"] ?></option>
									<?endforeach?>
								</select>
							</span>
						<?break?>
					...
					case 3://multiselect
						...
						<span class="vote-answer-item vote-answer-item-multiselect">
								<select
									name="vote_multiselect_<?= $arAnswer["QUESTION_ID"] ?>[]" <?= $arAnswer["~FIELD_PARAM"] ?>
									multiple="multiple">
									<?foreach ($arAnswer["MULTISELECT"] as $arMultiSelect):?>
										<option
											value="<?= $arMultiSelect["ID"] ?>" <?= (array_search($arMultiSelect["ID"], $value) !== false) ? 'selected="selected"' : '' ?>><?= $arMultiSelect["MESSAGE"] ?></option>
									<?endforeach?>
								</select>
							</span>
						<?break?>
					...
					case 4://text field
						...
						<span class="vote-answer-item vote-answer-item-textfield">
								...
								<input type="text" name="vote_field_<?= $arAnswer["ID"] ?>"
									   id="vote_field_<?= $arAnswer["ID"] ?>" <?
								?>value="<?= $value ?>"
									   size="<?= $arAnswer["FIELD_WIDTH"] ?>" <?= $arAnswer["~FIELD_PARAM"] ?> /></span>
						<?break?>
	...
	<button type="submit" name="vote" value="<?=GetMessage("VOTE_SUBMIT_BUTTON")?>" /><?=GetMessage("VOTE_SUBMIT_BUTTON")?></button>

	<?/*?>	<span class="vote-form-box-button vote-form-box-button-last"><input type="reset" value="<?=GetMessage("VOTE_RESET")?>" /></span><?*/?>

</form>

<a class="link icon icon16 result" href="<?=$arResult["URL"]["RESULT"]?>"><?=GetMessage("VOTE_RESULTS")?></a>