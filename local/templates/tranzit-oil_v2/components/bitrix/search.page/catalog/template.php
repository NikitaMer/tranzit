...
<?elseif(count($arResult["SEARCH"])>0):?>
    ...
    <?foreach($arResult["SEARCH"] as $arItem):?>  
        ...
        <?                              
        $arSelect = Array();
        $arFilter = Array("ID"=>IntVal($arItem['PRODUCT']['ID']), "ACTIVE"=>"Y");                              
        $res = CIBlockElement::GetList(Array(), $arFilter, false, Array(), $arSelect);
        while($ob = $res->GetNextElement()) { 
            $arFields = $ob->GetFields();  
        }                                   
        $db_old_groups = CIBlockElement::GetElementGroups(IntVal($arItem['PRODUCT']['ID']));           
        while($ob_group = $db_old_groups->GetNextElement()) {
            $ar_group = $ob_group->GetFields();        
            $ar_codes = explode("/", $ar_group['SECTION_PAGE_URL']);   
            $new_url = '/'.$ar_codes['1'].'/'.$ar_codes['2'].'/'.$arItem['PRODUCT']['CODE'].'/';             
            $arItem['URL'] = $new_url;                 
        }                                           
        ?>       
